...
/client/proc/RollCredits()
	...
	if(mob)
		...
		if(mob.get_preference_value(/datum/client_preference/play_lobby_music) == GLOB.PREF_YES)
			sound_to(mob, sound(null, channel = GLOB.CHANNEL_LOBBYMUSIC))
			if(GLOB.end_credits_song == null)
				var/title_song = pick('sound/music/utopia.ogg')
				sound_to(mob, sound(title_song, wait = 0, volume = 40, channel = GLOB.CHANNEL_LOBBYMUSIC))
			else if(get_preference_value(/datum/client_preference/play_admin_midis) == GLOB.PREF_YES)
				sound_to(mob, sound(GLOB.end_credits_song, wait = 0, volume = 40, channel = GLOB.CHANNEL_LOBBYMUSIC))
	sleep(50)
...
/proc/generate_titles()
	...
	for(var/mob/living/carbon/human/H in GLOB.alive_mobs|GLOB.dead_mobs)
		if(findtext(H.real_name,"(mannequin)"))
			continue
		if(H.is_species(/datum/species/monkey) && findtext(H.real_name,"[lowertext(H.species.name)]")) //no monki
			continue
